...
// ─── SINDy pass: export series → call sidecar → ingest candidate → update decay ─
...
export async function runSINDyPass(): Promise<PlatformDynamicsCandidate | null> {
  ...
  try {
    ...
    // Ingest into HellGraph as auditable artifact
    ingestPrometheusCandidate(candidate)

    // Apply adaptive decay if fit quality is acceptable (NMSE <= 0.1)
    ...
    return null  // sidecar unavailable — degrade gracefully
  }
}
