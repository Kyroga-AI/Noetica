...
MODELS = {
    ...
    # ── chemistry (failure-driven enrichment: the board's chemistry-numeric cell had a 48-pt oracle gap and
    #    the catalog held ONE chemistry law — compute abstained on every stoichiometry/molarity/pH question.
    #    These are textbook general laws captured from authoritative knowledge, NOT from the test items.) ──
    "Molarity":                ("M = n/V",                 "chemistry",           "M = n/V",
                                 ({"n": 0.5, "V": 2}, "M", 0.25)),
    "Moles from mass":         ("n = mass/MW",             "chemistry",           "n = mass/MW",
                                 ({"mass": 18, "MW": 18}, "n", 1)),
    "Dilution":                ("M2 = M1*V1/V2",           "chemistry",           "M1·V1 = M2·V2",
                                 ({"M1": 2, "V1": 0.5, "V2": 1}, "M2", 1)),
    "pH from [H+]":            ("pH = -log(H)/log(10)",    "chemistry",           "pH = -log10[H+]",
                                 ({"H": 1e-3}, "pH", 3)),
    "pH and pOH":              ("pH = 14 - pOH",           "chemistry",           "pH + pOH = 14",
                                 ({"pOH": 5}, "pH", 9)),
    "Henderson-Hasselbalch":   ("pH = pKa + log(A/HA)/log(10)", "chemistry",      "pH = pKa + log10([A-]/[HA])",
                                 ({"pKa": 4.74, "A": 1, "HA": 1}, "pH", 4.74)),
    "Boyle's law":             ("P2 = P1*V1/V2",           "chemistry",           "P1·V1 = P2·V2",
                                 ({"P1": 1, "V1": 2, "V2": 1}, "P2", 2)),
    "Combined gas law":        ("P2 = P1*V1*T2/(T1*V2)",   "chemistry",           "P1V1/T1 = P2V2/T2",
                                 ({"P1": 1, "V1": 1, "T1": 273, "T2": 546, "V2": 1}, "P2", 2)),
    "Rate law (first order)":  ("rate = k*A",              "chemistry",           "rate = k·[A]",
                                 ({"k": 0.1, "A": 2}, "rate", 0.2)),
    "Half-life (first order)": ("thalf = log(2)/k",        "chemistry",           "t-half = ln2/k",
                                 ({"k": 0.0693}, "thalf", 10)),
    "Beer-Lambert":            ("A = eps*l*c",             "chemistry",           "A = eps·l·c",
                                 ({"eps": 100, "l": 1, "c": 0.01}, "A", 1)),
    "Osmotic pressure":        ("Pi = M*R*T",              "chemistry",           "Pi = M·R·T",
                                 ({"M": 1, "R": 8.314, "T": 298}, "Pi", None)),
    "Freezing-point depression":("dTf = i*Kf*b",           "chemistry",           "dTf = i·Kf·b",
                                 ({"i": 1, "Kf": 1.86, "b": 0.5}, "dTf", 0.93)),
    "Percent yield":           ("pct = 100*actual/theo",   "chemistry",           "pct = 100·actual/theoretical",
                                 ({"actual": 8, "theo": 10}, "pct", 80)),
    "Nernst equation":         ("E = E0 - (R*T/(z*F))*log(Q)", "chemistry",       "E = E0 - (RT/zF)·lnQ",
                                 ({"E0": 1.1, "R": 8.314, "T": 298, "z": 2, "F": 96485, "Q": 1}, "E", None)),
