...
export type Tool = 'read_file' | 'write_file' | 'list_directory' | 'code_execute' | 'web_search' | 'generate_image' | 'remember' | 'ocr' | 'run_command' | 'registry_lookup'
...
const INTENTS: Intent[] = [
  { id: 0, name: 'build_implement', model: 'code', retrieval: 'kb', slots: ['target', 'requirements'], tools: ['registry_lookup', 'write_file', 'run_command', 'code_execute', 'read_file'], surface: 'code', skill: 'coding-agent', cues: /\b(build|create|implement|add (a|an|the)|set up|develop|scaffold|stand up|wire (up|in)|(write|create|make|generate) (me )?(a |an |some )?(\w+ )?(function|script|program|class|method|module|cli|command|app|code|snippet|regex|algorithm|parser|query))\b/i },
  { id: 1, name: 'fix_debug', model: 'code', retrieval: 'kb', slots: ['symptom', 'location'], tools: ['read_file', 'run_command', 'code_execute', 'write_file'], surface: 'code', skill: 'coding-agent', cues: /\b(fix|broken|does ?n'?t work|not working|error|crash(ed|ing)?|bug|fail(s|ed|ing)?|stuck|hang(s|ing)?|busted)\b/i },
  ...
  { id: 16, name: 'compute_math', model: 'reasoning', retrieval: 'program-aided', slots: ['expression'], tools: ['code_execute', 'registry_lookup'], surface: 'canvas', skill: 'analytics-agent', cues: /\b(compute|calculate|evaluate|solve for|integral|derivative|determinant|probability of|what'?s \d)\b/i },
