...
export type Tool = 'read_file' | 'write_file' | 'edit_file' | 'list_directory' | 'code_execute' | 'web_search' | 'generate_image' | 'remember' | 'ocr' | 'run_command' | 'scaffold_app' | 'registry_lookup' | 'render_chart' | 'brain_status' | 'update_self' | 'set_identity' | 'find_symbol' | 'execute_action'
...
const INTENTS: Intent[] = [
  { id: 0, name: 'build_implement', model: 'code', retrieval: 'kb', slots: ['target', 'requirements'], tools: ['scaffold_app', 'registry_lookup', 'render_chart', 'write_file', 'edit_file', 'run_command', 'code_execute', 'read_file', 'find_symbol'], surface: 'code', skill: 'coding-agent', cues: /\b(build|create|implement|add (a|an|the)|set up|develop|scaffold|stand up|wire (up|in)|(write|create|make|generate) (me )?(a |an |some )?(\w+ )?(function|script|program|class|method|module|cli|command|app|code|snippet|regex|algorithm|parser|query))\b/i },
  { id: 1, name: 'fix_debug', model: 'code', retrieval: 'kb', slots: ['symptom', 'location'], tools: ['read_file', 'edit_file', 'run_command', 'code_execute', 'write_file', 'find_symbol'], surface: 'code', skill: 'coding-agent', cues: /\b(fix|broken|does ?n'?t work|not working|error|crash(ed|ing)?|bug|fail(s|ed|ing)?|stuck|hang(s|ing)?|busted)\b/i },
  ...
  { id: 7, name: 'review_audit', model: 'reasoning', retrieval: 'kb', slots: ['target', 'criteria'], tools: ['read_file', 'code_execute', 'execute_action'], surface: 'govern', skill: 'governance-sentinel', cues: /\b(review|audit|harden|assess|critique|go over|reassess|gaps assessment)\b/i },
  ...
  { id: 9, name: 'self_identity', model: 'general', retrieval: 'self-model', slots: ['aspect'], tools: ['brain_status', 'update_self', 'set_identity'], surface: 'holographme', skill: 'governance-sentinel', cues: /\b(your ?self|how do you work|what are you|your (construction|architecture|design|self|repos?|stack|knowledge|brains?)|do you know yourself|who built you|update your(self| knowledge| brains?)|what do you know)\b/i },
  { id: 10, name: 'preferences_memory', model: 'general', retrieval: 'memory-write', slots: ['preference'], tools: ['remember', 'set_identity'], surface: 'notes', skill: 'memory-keeper', cues: /\b(remember (this|that)|my preference|i prefer|don'?t (do|add|use|ever)|from now on|always (use|do)|never (use|do)|my name is|my email is|set my profile)\b/i },
  { id: 11, name: 'configure_ops', model: 'code', retrieval: 'kb', slots: ['target', 'env'], tools: ['run_command', 'code_execute', 'list_directory', 'write_file', 'brain_status', 'update_self', 'execute_action'], surface: 'operate', skill: 'security-agent', cues: /\b(install|set ?up|configure|provision|deploy|brew |npm |pip |docker|podman|build the app|reinstall|reboot|update (your|the) (knowledge|brains?)|refresh your)\b/i },
  ...
  { id: 13, name: 'file_ops', model: 'code', retrieval: 'none', slots: ['path', 'operation'], tools: ['list_directory', 'read_file', 'write_file', 'ocr', 'find_symbol'], surface: 'operate', skill: 'coding-agent', cues: /\b(read the file|find (the )?files?|list (the )?files?|directory|folder|move (the )?file|in my (downloads|documents|desktop)|(read|extract|get) (the )?text (from|in)|what does (this|the) (image|screenshot|photo)|ocr)\b/i },
  // The bare `status` keyword used to match ANY message containing that word anywhere — including
  // task-progress check-ins like "any status update on the research?" — which misrouted them into this
  // SYSTEM health-check intent (its only tool is brain_status, which reports on knowledge-pack versions,
  // not on conversational tasks). Narrowed so "status" only fires when it's asking about Noetica itself.
  { id: 14, name: 'status_check', model: 'general', retrieval: 'status', slots: ['component'], tools: ['brain_status'], surface: 'operate', skill: 'governance-sentinel', cues: /\b(is it (running|working|up|on)|(your|noetica'?s|system|app|agent) status|status check|does it work|did it (work|boot|build)|health\s*check|still (broke|busted|working)|is your knowledge (current|up to date|loaded))\b/i },
  { id: 15, name: 'code_review', model: 'code', retrieval: 'kb', slots: ['diff'], tools: ['read_file', 'find_symbol'], surface: 'code', skill: 'coding-agent', cues: /\b(review (this|my|the) (code|pr|diff|change)|code review|look at (this|my) (code|pr))\b/i },
  ...
  { id: 21, name: 'meta_capability', model: 'general', retrieval: 'self-model', slots: [], tools: ['brain_status'], surface: 'holographme', skill: 'governance-sentinel', cues: /\b(what can you do|your capabilities|what are you capable|how can you help|what do you do)\b/i },
...
export function classifyIntent(text: string, ctx: { hasDoc?: boolean } = {}): IntentPlan {
  ...
  if (!best) {
    ...
    // qa_over_doc just because a doc exists — that path answers ONLY from retrieved context and REFUSES anything
    // not in it ("what year was hurricane helene" → "not in the documents"), even when the model plainly knows
    // it. A genuine question ABOUT an uploaded doc is already caught above (hasDoc + '?' + weak-intent → qa_over_doc).
