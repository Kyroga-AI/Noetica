...
export function CitizenOnboardingWizard({ onComplete, onDismiss }: Props) {
  ...
  // Escapable at any point — mark it seen so it doesn't nag again, then close. Onboarding is an
  // offer, not a gate.
  function dismiss() {
    localStorage.setItem('noetica:citizen:onboarded', '1')
    onDismiss()
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={dismiss}>
      <div className="relative w-full max-w-lg rounded-2xl border border-[var(--color-border-primary)] bg-[var(--color-background-primary)] shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Close — onboarding is skippable at any step (click outside, ✕, or Esc) */}
        <button
          onClick={dismiss}
          title="Close (Esc)"
