...
export function CommandPalette({
  ...
  const actions = useMemo<PaletteAction[]>(() => [
    ...
    { id: 'surface_chat',         label: 'Switch to Chat',             shortcut: '⌘1', group: 'Surfaces', run: () => { onSwitchSurface('chat');       onClose() } },
    ...
    { id: 'surface_cowork',       label: 'Switch to Collaborate',      shortcut: '⌘4', group: 'Surfaces', run: () => { onSwitchSurface('cowork');      onClose() } },
    ...
    { id: 'surface_studio',       label: 'Switch to Studio — prompt workbench + model compare', group: 'Surfaces', run: () => { onSwitchSurface('studio'); onClose() } },
    { id: 'surface_rag',          label: 'Switch to RAG Inspector — retrieval debug',           group: 'Surfaces', run: () => { onSwitchSurface('rag');    onClose() } },
    { id: 'surface_lab',          label: 'Switch to Capabilities — investigation/reasoning/safety', group: 'Surfaces', run: () => { onSwitchSurface('lab'); onClose() } },
    { id: 'surface_docs',         label: 'Switch to Documents — view docs/sheets',              group: 'Surfaces', run: () => { onSwitchSurface('docs');   onClose() } },
    { id: 'surface_jitsi',        label: 'Switch to Video — workroom calls',                    group: 'Surfaces', run: () => { onSwitchSurface('jitsi');  onClose() } },
        ...
        <div className="max-h-80 overflow-y-auto py-2">
          {filtered.length === 0 ? (
            ...
            Object.entries(groups).map(([group, items]) => (
              <div key={group}>
                <div className="px-4 py-1.5 text-[11px] font-semibold text-[var(--color-text-tertiary)]">
                  {group}
                </div>
                {items.map((action) => {
